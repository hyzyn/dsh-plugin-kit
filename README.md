...
- **做什么**：在 Web GUI 侧边栏加一个「全局搜索」入口，输入关键词即可全文搜索历史会话。
- **怎么用**：安装后在侧边栏「新建会话」下方点击 / 聚焦全局搜索框 → 输入关键词 → 点击会话会打开并尝试定位到匹配文字。
- **支持**：历史会话全文搜索（走 DSH 自带 sessionQuery 索引）；结果数量可配置；结果关键词高亮。
- **存哪里**：无独立配置。
