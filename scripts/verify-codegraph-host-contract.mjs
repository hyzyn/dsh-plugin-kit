 ...
 *
 * 为什么需要它（历史教训）：`client.js` 的 combo 直链失效、CLI 探测失败原因被吞
 * 这类问题**单测一律绿的**——单测用 fake req/res，浏览器半体的供给路径根本不在其中。
 * 本脚本与 `scripts/verify-codegraph-indexforce.mjs` 同源（它只验 indexForce 一件事），
 * 这里把**路由面 + 供给面**整体验一遍。
 *
 *
 * ⚠️ 编号存疑（2026-09-25 复核）：本注释原先写的是「D10（combo 直链失效）、D3（探测失败原因被吞）」，
 * 但 `D10` / `D3` 在**三份台账里都查不到**，`git log -S` 全历史亦零命中（它们早于 codegraph
 * 启用 `CGxx` 编号）。教训本身成立、回归点也真实，但**编号是悬空的**——本次只标注、不猜。
 * 要修的话二选一：归到某个 `CGxx`，或直接去掉编号只留描述。下方 325/330/331 行的 `D10` 同理。
