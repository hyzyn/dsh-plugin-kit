...
- **模型能直接引用今天的新闻**：汇总结果作为 `systemPrompt` section 注入，问「今天有什么值得读」就有内容。
- **每天自动生成**：定时抓取 + 按分类汇总；弹窗里能搜索、按分类筛、复制 Markdown。
- **订阅源在 GUI 里管**：内置渠道 + 自定义源，RSSHub 精选目录一键添加，支持 OPML 导入导出。
- **失败可见但不致命**：个别源挂了只标注它自己，其余照常出；可选 AI 摘要，失败时回落到原文摘要并说明原因。
