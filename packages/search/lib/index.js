 ...
 */
let promptStoreCache;
/** 文件指纹：mtimeMs + size；文件不存在或 statSync 失败返回 undefined（按不存在处理）。 */
function promptFileFingerprint(file) {
    try {
...
function includesText(haystack, query) {
    if (!haystack)
        return false;
...
function searchPrompts(rawQuery, limit) {
    ...
    for (const prompt of managed.store.prompts) {
        ...
        const version = prompt.versions.find((item) => includesText(item.content, query) || includesText(item.label, query) || includesText(item.note, query));
        if (!nameHit && !descriptionHit && version === undefined)
            continue;
...
function getCachedDocs(sessionId) {
...
function setCachedDocs(sessionId, entry) {
    const previous = sessionDocCache.get(sessionId);
    if (previous !== undefined)
        sessionDocBytes -= previous.bytes;
...
function setCachedResult(key, hits) {
...
async function searchSessions(ctx, rawQuery, limit, signal, maxScanSessions = MAX_SCAN_SESSIONS) {
    ...
    if (cached !== undefined)
        return filterVisibleSessionHits(ctx, cached, signal);
    let hits = [];
    if (typeof sessionQuery.searchSessions === 'function') {
        try {
            ...
            hits = (page.items ?? []).slice(0, limit).flatMap((hit) => {
                ...
                if (!id)
                    return [];
                if (isSubagentHeader(hit.header))
                    return [];
                return [{
                        id,
...
async function scanOneSession(sessionQuery, sessionId, filter, query) {
    try {
        ...
        const doc = cached.docs.find((item) => filter.test(item.text) && item.surface === 'current' && (item.type === 'user/message' || item.type === 'assistant/message')) ?? cached.docs.find((item) => filter.test(item.text));
        if (doc === undefined)
            return undefined;
        return {
            id: sessionId,
...
let visibleSessionIdsCache;
async function getVisibleSessionIds(ctx, signal) {
    const cached = visibleSessionIdsCache;
    if (cached !== undefined && Date.now() - cached.at <= VISIBLE_CACHE_TTL_MS)
        return cached.ids;
    const ids = new Set();
    ...
    if (persistence?.list !== undefined) {
        try {
            ...
            for (const meta of cold) {
                ...
                if (isSubagentHeader(header) || isSubagentHeader(meta))
                    continue;
    ...
    if (ids.size > 0)
        visibleSessionIdsCache = { ids, at: Date.now() };
    return ids;
}
async function filterVisibleSessionHits(ctx, hits, signal) {
...
function searchMcpTools(ctx, rawQuery, limit) {
    const query = normalizeQuery(rawQuery);
    if (query === '')
        return [];
    ...
    for (const schema of schemas) {
        ...
        if (!schema.name.startsWith('mcp__'))
            continue;
        if (!includesText(schema.name, query) && !includesText(schema.description, query))
            continue;
 ...
 */
function listPanels(ctx) {
    const loaded = getLoadedRegistryNames(ctx);
    const items = [];
    for (const panel of PANEL_DIRECTORY) {
        if (panel.registryName !== undefined && !loaded.has(panel.registryName))
            continue;
        items.push({
            id: panel.id,
            kind: panel.kind,
...
function searchPanels(ctx, rawQuery, limit) {
    const query = normalizeQuery(rawQuery);
    if (query === '')
        return [];
    const hits = [];
