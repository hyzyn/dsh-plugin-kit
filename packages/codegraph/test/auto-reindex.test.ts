...
describe('P1 索引生命周期：自动重建', () => {
  it('默认关：过期也不重建（一行 CLI 都不调）', async () => {
    ...
    await wait(300)
    expect(require('node:fs').readFileSync(logFile, 'utf8').trim()).toBe('')
  ...
  it('开启且索引过期：先 status 再 index（是重建，不是 sync）', async () => {
    ...
    mount_.userMessage(dir)
    // 等异步链跑完（status → 判定 → index）
    for (let i = 0; i < 40 && !require('node:fs').readFileSync(logFile, 'utf8').includes('index'); i++) await wait(50)
    const calls = require('node:fs').readFileSync(logFile, 'utf8').trim().split('\n').filter(Boolean)
...
`)
    ...
    for (let i = 0; i < 40 && !require('node:fs').readFileSync(logFile, 'utf8').includes('index'); i++) await wait(50)
    await wait(200)
    expect(require('node:fs').readFileSync(logFile, 'utf8')).toContain('index')
    rmSync(dir, { recursive: true, force: true })
  })
})
