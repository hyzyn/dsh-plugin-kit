 ...
 */
import { REL_LIMIT, adoptionText, buildQuery, fmtBytes, fmtNum, fmtTime, nextRetryDelayMs, seenAgoText, shortPath, staleReasons, truncationNote } from './pure.js'
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     *
     * `action` 是「属于本组的**单个辅助动作**」——典型例子是「刷新」：它重新读取的正是
     * 「索引状态」这一组的数据，原先却放在「索引维护 → 查看与诊断」里，隔着一整屏
     * （用户直接问「这个状态和刷新状态有关系吗」）。放到组头右侧，关系一眼可见。
     *
     * **不要把主操作组放进这个槽**（被问过「生命周期那几个按钮要不要也放到头上」）：
     *
     *   - 组头是「标签 + 细线 + 一个紧凑控件」，细线靠 flex:1 占满剩余宽度；塞进 4 个
     *     文字按钮（约 390px）会把细线挤没，窄栏下还会折成两截，看起来像布局坏了；
     *   - 更要紧的是**视觉重量**：Sync 是这张卡片的主按钮（cg_btn 实心蓝），进组头就
     *     降级成与标签并排的小控件，主次颠倒；而「索引维护」这一节的**内容本来就是
     *     这些动作**，组头只负责给它命名，不该把内容搬到名字旁边。
     *
     * 判据：**一个**动作、且是辅助性的（图标 / 小按钮）→ 放组头；**一组**动作、
     * 或这组的主体就是它们 → 留在组内。
     *
            ...
            children: jsxs('div', {
              ...
              children: [
                ...
                group('搜索与查询', [
                  ...
                  // 三个查询按钮的说明行。
                  //
                  // 为什么需要：它们的信息原先只在 `title` 悬停提示里——用户直接问
                  // 「这两个按钮做啥的」。搜索/探索/上下文都是「拿同一个关键词查」，但**产物
