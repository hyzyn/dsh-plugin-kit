...
3. 或者：**保持现状**，把 `projectPath` 的使用指引写得更显眼（usage 段已提到），并把这次调研结论并入 [`docs/adoption-audit.md`](./adoption-audit.md)。
...
> **状态（2026-09-25 补注）**：本节是 **2026-09-22 动工前**的时点记录，原文一字未改。
> 其中第 1、2、4、5 个问题已在下方 [「实现记录（已完成）」](#七实现记录已完成) 的对照表里回答
> （默认值取 `managed`；新增依赖从 3 个缩到 1 个；互斥靠**挂起**因而可逆；依赖不是硬门槛）；
> **第 3 个（三平台验收标准）那一节没有明确回答**，保留待答。
...
> **状态（2026-09-25 补注）**：本节是 **2026-09-22 动工前**的时点记录，原文一字未改。
> 这 6 步**已全部执行完毕**——下方「实现记录（已完成）」开头那句「按上面的顺序做完了」指的就是本节；
> 保留它是为了对照「方案原写」与「实际落地」的差异。
