...
- **Measured numbers: see [docs/adoption-audit.md](./docs/adoption-audit.md)**: traced back over 177 real historical sessions — 23% session-level, 28.8% narrow, 47% over the last three days — plus a quality sample of the `query` calls codegraph received (34/47 carried a concrete symbol name).
...
- **DSH**: the host version matrix (every tier states **how it was verified** — do not dress a regression run up as a compatibility claim):
  ...
  - **`0.1.7-rc.2` (the current baseline, measured 2026-09-25)**: the cohort moved from rc.1 to rc.2, raising the `peerDependencies` and `dsh.engines.dsh` lower bounds together. `node scripts/verify-codegraph-host-contract.mjs --profile test --port 3087` (this machine's global dsh already is rc.2, so no `--dsh-bin` / `--runtime-store` needed) — **42/42 passed**: all 25 routes, the POST / loopback gates, `/diagnose` sections, the browser-half artifact being servable, the managed MCP row written into the isolated home patch, and `mcpScope` reversible between `per-agent` and `managed` with the real `~/.dsh/cordis.patch.yml` byte-identical. Two more: a real host loading the test profile mounts **all 10 plugins with 0 lines of `disabling profile plugin row`**; and `pnpm -r typecheck` green across the workspace, `vitest` **960/960**, `pnpm -r build` green. This round changed no plugin runtime behavior (only compatibility declarations, docs and the lockfile). Per-file comparison of the lib artifacts from rc.1 → rc.2: `dsh-tools` only adds an optional `PreToolDecision.displayReason` (this repo constructs no such decision), `dsh-session-query` only gained JSDoc, `dsh-client-ui-theme` only added design tokens, and `dsh-mcp-client` / `dsh-host-webserver` / `dsh-subprocess-local` / `dsh-credentials` / `dsh-base` / `dsh-settings` / `dsh-web-app` / `dsh-headless` are byte-identical; `dsh-app-boot` removed `skippedProfileBundles` and changed `generateConfigSchema`'s signature (dropping the leading `binName`), but that is an internal surface between DSH's own CLI and app-boot that this repo never references.

    > ⚠️ **Note (left as-is) — the two test totals are not from the same run**: this entry records the
    > count at the **rc.2 verification point** (`960/960`); after this round of project-level documentation
    > work the current value is **56 files / 965 tests** repo-wide (`packages/codegraph` itself:
    > 14 files / 317 tests). **Neither number is changed** — the current value is the
    > [ROADMAP.md “2026-09-25 re-measurement” table](./ROADMAP.md#基线本次实测).
    >
    > The same goes for the “25 routes” figure here: **compute it with the command** (see the ROADMAP
    > re-measurement table); do not treat it as a contract.
  - **`0.1.6-alpha.2` / `0.1.5-rc.2` / `0.1.0-rc.7` (historical baselines, no longer supported)**: those tiers still had the old `register(ns, schema)` API for the `settings` service, whereas the 0.1.7 line replaced it with `SettingsForms` (`describe/update` plus a volatile exported Config); this package's `peerDependencies` lower bound `^0.1.7-rc.2` also makes a new host refuse to load it onto an old one, both before install and at startup. The evidence collected for those tiers at the time (`0.1.6-alpha.2` 40/40, `0.1.5-rc.2` no script, `0.1.0-rc.7` audit baseline) stays in git history for reference only.
  - `package.json` declares `peerDependencies: { "@deepseek-ai/dsh": "^0.1.7-rc.2" }` (with `optional` in `peerDependenciesMeta`, purely to quiet pnpm's unmet-peer noise). From DSH 0.1.7-rc.1 on, compatibility is judged from this **before install** and **at startup**: an incompatible pair throws `incompatible-version` on install, and at startup the whole row is marked `disabled`. `dsh.engines.dsh: ">=0.1.7-rc.2"` is kept as well, as the **market display slot** (the host does not read it — see the next bullet), and `scripts/check-dsh-peers.mjs` asserts that the two lower bounds agree.
  - **Why peer rather than `engines.dsh`**: `dsh-app-boot` walks only `peerDependencies` entries named `@deepseek-ai/dsh` or starting with `@deepseek-ai/dsh-`, judging them with `semver.satisfies(runtime, range, { includePrerelease: true })` (**prereleases take part in range matching**). **`engines.dsh` is still declared, though** — the host does not read it, but the plugin market / community entries display compatibility from it, so it is kept in the standard form `>=0.1.7-rc.2` (the market resolver only accepts `>=X.Y.Z[-prerelease]`) and made to agree with the peer lower bound. Evidence: `evaluatePluginCompatibility` in `dsh-app-boot/lib/index.js`, and the app-boot README's own wording that these checks use the peer declaration rather than `engines.dsh`.
  - **Why the range is `^0.1.7-rc.2`**: with prereleases taking part in matching, `^0.1.7-rc.2` = `>=0.1.7-rc.2 <0.2.0`, so `0.1.7` final and later `0.1.7-rc.N` / `0.1.8` all pass, while `0.1.7-rc.1`, `0.1.7-alpha.2` and `0.1.6-*` do not — consistent with the "0.1.7-rc.2 and later only" policy. To temporarily admit another version combination, write the exemption into the profile's own `compatibility.json` (`dsh plugin --profile <p> allow-version <pkg@ver> --dsh-version <ver> --accept-risk`) rather than loosening the range here.
...
- [docs/p0-plan.md](https://github.com/hyzyn/dsh-plugin-kit/blob/main/packages/codegraph/docs/p0-plan.md): the full plan and measurements for per-agent scoped MCP mounting (the design doc of a shipped item, **moved into the package's `docs/`**).
- [docs/adoption-audit.md](https://github.com/hyzyn/dsh-plugin-kit/blob/main/packages/codegraph/docs/adoption-audit.md): adoption measurements — the numbers computed from 177 real historical sessions, the tradeoffs between the two denominators, and what they imply for the roadmap (**moved into the package's `docs/`**).
