 ...
 */
function detectExternalCodegraphRow(lines) {
    ...
    try {
        ...
        for (const entry of parsed) {
            ...
            if (!Array.isArray(inserted))
                continue;
            const found = inserted.find((row) => typeof row === 'object' && row !== null && isCodegraphServerRow(row));
            if (found !== undefined)
                return found;
        }
    }
 ...
 */
function hasExternalCodegraphRow() {
    try {
 ...
 */
function locateRowKeyEdits(lines, range, key, value) {
 ...
 */
function findRowKeyLine(lines, range, key) {
    const bodyEnd = range.end === range.start + 1 && range.markerEnd === '' ? range.start + 1 : range.end - 1;
...
const isTrueScalar = (text) => /^['"]?true['"]?$/i.test(text.trim());
/**
 ...
 */
function readDisabledState(lines, range) {
    const hit = findRowKeyLine(lines, range, 'disabled');
    if (hit === null)
        return null;
    if (hit.keyIndex === -1)
        return { present: false, disabled: false, ours: false };
 ...
 */
function locateSuspendEdits(lines, range, suspend) {
    const hit = findRowKeyLine(lines, range, 'disabled');
    if (hit === null)
        return [];
    ...
    if (mcpRow !== undefined && mcpRange) {
        if (decision.suspendGlobal) {
            ...
            if (!dryRun) {
                ...
                if (restore.length > 0)
                    replacements.push({ kind: 'lines', range: mcpRange, edits: restore });
            }
 ...
 */
export function shouldRecheckPatchWrite(beforeStamp, afterStamp, attempt) {
...
function syncMcpRowOnDisk(decision) {
    ...
    try {
        for (let attempt = 0;; attempt++) {
            ...
            if (shouldRecheckPatchWrite(patchStamp(before), patchStamp(after), attempt))
                continue;
    ...
    return [
        ...
        },
        {
            ...
            handler: async (req, res) => {
                ...
                if (body.mcpScope !== undefined) {
                    ...
                    if (typeof body.mcpScope !== 'string' || !MCP_SCOPE_MODES.includes(body.mcpScope)) {
        ...
        },
        {
            ...
            handler: async (req, res) => {
                ...
                if (cliProbe.get().available === undefined)
                    await cliProbe.reprobe();
...
const plugin = definePlugin({
    ...
    apply(ctx, rawConfig) {
        ...
        try {
            const on = ctx.on;
            on.call(ctx, 'agent/created', (...args) => {
            ...
            on.call(ctx, 'agent/disposed', (...args) => {
                const agent = args[0]?.agent;
                if (agent === undefined)
                    return;
         ...
         */
        let appliedScopeMode;
        const applyScopeMode = (runtime, wanted, decision) => {
            const effective = decision.mode;
            if (appliedScopeMode === effective)
                return;
