...
function validatePromptInput(raw, existing) {
    ...
    if (name.length > MAX_NAME_LENGTH)
        return { error: 'name 不能超过 ' + MAX_NAME_LENGTH + ' 字符' };
    ...
    if (input.ab === undefined || input.ab === null) {
        ...
        if (typeof input.ab !== 'object' || Array.isArray(input.ab))
            return { error: 'ab 必须是对象' };
        const rawAb = input.ab;
        ...
        if (!versions.some((version) => version.id === aVersionId))
            return { error: 'A/B 的 A 版本不存在: ' + aVersionId };
        if (!versions.some((version) => version.id === bVersionId))
            return { error: 'A/B 的 B 版本不存在: ' + bVersionId };
        if (enabled && aVersionId === bVersionId)
            return { error: 'A/B 两个版本不能相同' };
        if (!Number.isInteger(aWeight) || aWeight < 0 || aWeight > 100)
            return { error: 'aWeight 必须是 0~100 的整数' };
    ...
    if (!PROMPT_ID_RE.test(id))
        return { error: '非法 prompt id: ' + JSON.stringify(id) };
    if (existing !== undefined && existing.id !== id)
        return { error: '不能修改 prompt id' };
    return {
        prompt: {
            id,
            name,
...
function validateImportPayload(raw) {
    ...
    for (const rawPrompt of rawPrompts) {
        ...
        if (seenIds.has(validated.prompt.id))
            return { error: '导入数据里重复的 prompt id: ' + validated.prompt.id };
...
function resolveActiveText(store) {
    const empty = { promptId: null, variantId: null, text: '', ab: null };
...
function buildSystemPromptText(store) {
...
function makeRoutes(ctx, onChanged) {
    ...
    return [
        ...
        },
        {
            ...
            handler: async (req, res) => {
                ...
                const existing = typeof body.prompt === 'object' && body.prompt !== null
                    ? managed.store.prompts.find((prompt) => prompt.id === body.prompt.id)
                    : undefined;
                ...
                if (!nextStore.prompts.some((prompt) => prompt.id === validated.prompt.id)) {
                    ...
                    if (nextStore.activePromptId === null)
                        nextStore.activePromptId = validated.prompt.id;
                }
                try {
        ...
        },
        {
            ...
            handler: async (req, res) => {
                ...
                const managed = readManagedStore();
                if (promptId === '') {
                    const nextStore = { ...managed.store, activePromptId: null };
                    try {
                        writeManagedStore(nextStore);
                    }
                    catch (error) {
...
export function apply(ctx, config) {
    ...
    let announcementDisposer = null;
    const refreshSystemPrompt = () => {
        if (!applyToSystemPrompt || systemPromptApi === null)
            return;
