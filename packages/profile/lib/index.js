...
function readProfileRuntime(name) {
...
function writeProfileRuntime(name, port) {
    const file = runtimeFilePath(name);
    if (port === null) {
        if (existsSync(file))
    ...
    if (packageJson) {
        try {
            ...
            if (Array.isArray(manifest.dsh?.profile?.bundles)) {
                bundles = manifest.dsh.profile.bundles.filter((item) => typeof item === 'string');
            }
            dependencies = Object.fromEntries(Object.entries(manifest.dependencies ?? {}).map(([key, value]) => [key, String(value)]));
        }
        catch {
...
function createProfile(name, template) {
    const dir = profileDir(name);
    ensureProfilesRoot();
 ...
 */
async function installProfileDependencies(dir) {
    const outcome = await new Promise((resolve) => {
        ...
        child.on('error', (error) => resolve({ code: null, error: error.message, output }));
        child.on('close', (code) => resolve({ code, output }));
    });
    if (outcome.error !== undefined)
        throw new Error('复制后安装依赖失败: ' + outcome.error);
    if (outcome.code !== 0) {
            ...
            handler: async (req, res) => {
                ...
                try {
                    ...
                    if (portResult.port !== undefined)
                        writeProfileRuntime(name, portResult.port);
                }
                catch (error) {
