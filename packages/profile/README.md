...
- **新建**：三种模板可选
  - 默认：`@deepseek-ai/dsh-base`
  - `web`：base + `@deepseek-ai/dsh-web-app`
  - `headless`：base + `@deepseek-ai/dsh-headless`
- **复制**：整目录复制（跳过 `node_modules` 与 `pnpm-lock.yaml`），manifest 同步改名为 `dsh-profile-<name>`
...
- `POST /api/dsh-profile/create` —— 新建（`name` + 可选 `template`）
...
- 复制自动跳过 `node_modules` 与 `pnpm-lock.yaml`，不搬运安装产物
- 删除为递归删除、不可撤销，面板内会二次确认
