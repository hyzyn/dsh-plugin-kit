 ...
 * 变成一个没必要的报错。
 */
export function emptyServersRejection(incoming, existing, clearAll) {
    if (incoming > 0 || clearAll === true || existing === 0)
        return undefined;
 ...
 */
export function externalNameRejection(incoming, existing, external) {
    ...
    for (const row of incoming) {
        ...
        if (hit === undefined)
            continue;
        if (alreadySaved.get(row.id) === row.serverName)
            continue;
    ...
    return [
        ...
        },
        {
            ...
            handler: async (req, res) => {
                ...
                const clash = externalNameRejection(rows.map((row) => ({ id: row.id, serverName: row.config.serverName })), managed.rows.map((row) => ({ id: row.id, serverName: row.config.serverName })), externalMcpEntries(ctx, new Set(managed.rows.map((row) => row.id))));
                if (clash !== undefined) {
