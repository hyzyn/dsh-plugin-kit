...
> 尚未实测。
>
> **行号锚点冻结在审计时点（`17657205`）**：修复波（`be28ae6e`）让 `src/index.ts` 等文件长了
> 数百行，同一行号已不再指向本文所述的旧代码，因此文中引用一律只保留「文件:行号」文本、
> 不做行内跳转（避免点了跳到无关位置）。要定位修复后的实现，请检索条目里的函数名 / 代码
> 片段，或 `git show be28ae6e -- <file>` 看该文件的完整改动。
