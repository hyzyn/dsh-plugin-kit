...
> 修法讨论）已移出正文 —— 想看它：`git show 9a14abad:packages/tty/DEFECTS.md`
> （即 `d5b85074^`，496 行，含「附录：审计原文」全 48 条 + 前置修复）。
...
>
> 审计基线：v0.18.3（审计时 HEAD `17657205`，该 sha 已随历史重写失效、仅存记录）｜ 修复波 / 发布：tag `v0.1.36` → tty **0.19.0**
...
>
> **2026-09-20 复核实测**（HEAD `18df8e76`，放开沙箱后跑真实 PTY）：vitest **215（16 文件）** 全绿；
...
**已修 55 / 待修 0**（D01–D48 审计波 + D49/D50 线上反馈 + D51–D55 复核实测发现）。索引表**不写行号、也不保留修复提交
sha** —— 修复后代码移了位、有的整段被删或重写，审计时点的行号只会误导；而**本仓库的历史被重写
过**（当时记下的 sha 大多已不可解析，实测只有 D49/D50 的 `c46909b6` 还在），所以回溯入口统一改成
...
> 这两条不是那次审计的产物：0.19.2 之后由用户截图上门，修在 `c46909b6`，随后由 `v0.1.39` 发版
> 提交 `fae01ccf` 带出 **tty 0.19.3**（`c46909b6` 是本文里唯一还能在本仓库解析的 sha）。
...
- **根因**：一次脱敏（`92bf20a9` 那批，`filter-repo` 重写历史 + 补发版本）把示例 IP 换成文档
...
> 括号里的数字是 **2026-09-20 在本机实测**的值（HEAD `18df8e76`）；脚本会随功能增长，别把数字
> 当契约，看的是「全绿」。
