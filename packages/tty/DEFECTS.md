...
>
> **怎么读**：前半是「现状 / 索引 / 待办」（活的部分），48 条的完整原文冻结在文末
> 「附录：审计原文」。条目里的 `[x]` 表示「已修并入库」，**不等于**「当前行为已验证」——
> 想了解现在的行为请读 README 与代码。
>
...
**已修 48 / 待修 0**（D01–D48）——索引见下，完整原文见文末附录。被代码直接引用的 15 个编号
在最后一列标 ✓：改这些行为前先读附录里对应的「为什么」。
...
> 48 条按审计时的严重度分组；证据里的「文件:行号」是**审计时点**快照（修复后行号已
> 漂移，故不做行内跳转）。要定位修复后的实现，请检索条目里的函数名 / 片段，或
> `git show <修复提交> -- <file>`。

### 前置修复（`17657205`，缺陷编号之前的那一批）

**注意**：下面 6 条属于「状态胶囊」那轮修复——即审计开始前就已落地的部分，
它们与 D01–D48 是两批，不要当成审计发现。

- `17657205` fix(tty): 状态胶囊归属跟着活动标签；补契约返回值与文档 —— 即下面 6 条。
- `be28ae6e` fix(tty): 修复 43 项缺陷（D01–D37、D39–D44）；0.19.0 发版前置 —— 即 D01–D44 的勾选项。
...
- [x] **D01 SFTP 覆盖上传非原子，失败即毁原文件** —— `flags:'w'` 先截断同名文件，全程无临时文件
      ...
      证据：[src/sftp.ts:343](src/sftp.ts)、[src/index.ts:2712-2720](src/index.ts)；
      `sftp_write` 同型 [src/index.ts:3329-3332](src/index.ts)。✅ 复读
...
- [x] **D02 目录直传跟随符号链接 → 目录环无限递归** —— `sftp.stat`（跟随软链，lstat 才不跟随）判
      ...
      直到 ENAMETOOLONG 或磁盘被灌满。证据：[src/sftp.ts:408-419](src/sftp.ts)、
      [src/sftp.ts:493-505](src/sftp.ts)、[src/sftp.ts:369](src/sftp.ts)。✅ 复读
...
- [x] **D03 SSH 明文口令落浏览器存储** —— `password`/`passphrase` 随 `spawnSpec` 进
      ...
      同源脚本可读。
      证据：[client-src/index.js:3009-3013](client-src/index.js)、
      [client-src/index.js:820-824](client-src/index.js)、
      [client-src/index.js:842-848](client-src/index.js)、
      [client-src/index.js:2737](client-src/index.js)。
...
- [x] **D04 `tty_capture{last:true}` 无在途信号 → agent 拿到上一条命令的结果** —— 只读
      ...
      时返回形状正常的旧结果，agent 无从察觉。证据：[src/index.ts:2991-2997](src/index.ts)、
      [src/index.ts:498-510](src/index.ts)。
      修法：返回值带 `inProgress`（或 D 未到时报 `stale:true`）。
- [x] **D05 两处截断方向相反，恰好丢最近输出** —— `lastCommand.output` 在
      [src/index.ts:576](src/index.ts) 保尾，随后 [src/index.ts:2996](src/index.ts)
      又 `slice(0, 128K)` 保头；`tty_screen` 同病 [src/index.ts:3038](src/index.ts)
...
- [x] **D06 在途 spawn 不与 WS 连接绑定 → 僵尸会话** —— 会话在 `await spawnTerminal/spawnSsh`
      **之后**才进本连接 `local` 表，而 `cleanupAll` 只遍历该表 → 断线发生在 spawn 在途时
      ...
      [src/index.ts:1395-1442](src/index.ts)、[src/index.ts:1497-1548](src/index.ts)、
      [src/index.ts:1181-1204](src/index.ts)、[src/index.ts:1411](src/index.ts)、
      [src/index.ts:1628-1631](src/index.ts)、[src/index.ts:947](src/index.ts)。✅ 复读
...
- [x] **D07 `session.clients` 只用 clientSid 做键 → 跨连接互相踩** —— 浏览器「复制标签页」/ 会话
      ...
      证据：[src/index.ts:1247](src/index.ts)、[src/index.ts:1633](src/index.ts)、
      [src/index.ts:1187](src/index.ts)、[client-src/index.js:1138](client-src/index.js)。
...
- [x] **D08 `reconnectGraceSec` 热改为 0 后老孤儿永不回收** —— `reapOrphans` 在 `graceMs <= 0`
      ...
      证据：[src/index.ts:943-944](src/index.ts)、[src/index.ts:1194-1199](src/index.ts)、
      [src/index.ts:861-863](src/index.ts)。
...
- [x] **D09 `kill` 帧缺「孤儿」前提，可杀任意活跃会话** —— 注释写的是「前连接已断的孤儿也允许
      ...
      凭 sid 就能 SIGKILL 别的窗口正在跑的构建 / 长任务。证据：[src/index.ts:1591-1602](src/index.ts)。
...
- [x] **D10 `tty_expect` 的 acc 无界增长 + 并发无上限** —— `acc += decoder.write(chunk)` 从不裁剪
      ...
      证据：[src/index.ts:3095-3101](src/index.ts)、[src/index.ts:3086-3112](src/index.ts)、
      [src/index.ts:3079-3080](src/index.ts)。
...
- [x] **D11 `spawnSsh` 的 channel Promise 无超时兜底，可永久挂起且无取消入口** ——
      ...
      去取消。证据：[src/ssh.ts:348-462](src/ssh.ts)、[src/ssh.ts:237](src/ssh.ts)。
...
- [x] **D12 known_hosts 导入 → 假 MITM 告警并拒绝连接** —— 每 `host:port` 只留文件中第一条指纹
      ...
      证据：[src/known-hosts.ts:108-114](src/known-hosts.ts)、[src/ssh.ts:290-298](src/ssh.ts)、
      [src/ssh.ts:440-442](src/ssh.ts)。
...
- [x] **D15 隧道不会收敛** —— ①连接簿条目被删 / 改名后 `resolveBook` 返回 undefined 只走
      ...
      证据：[src/tunnels.ts:214-218](src/tunnels.ts)、[src/tunnels.ts:374-391](src/tunnels.ts)、
      [src/tunnels.ts:68-86](src/tunnels.ts)、[src/tunnels.ts:182-206](src/tunnels.ts)、
      [src/tunnels.ts:333-340](src/tunnels.ts)。
...
- [x] **D16 SSH 认证与错误文案误导** —— ①`env:VAR` 解析到**空串**时被当成「未设置」，报错指向
      ...
      证据：[src/ssh.ts:182-199](src/ssh.ts)、[src/ssh.ts:244-245](src/ssh.ts)、
      [src/probe.ts:170-174](src/probe.ts)、[src/probe.ts:55](src/probe.ts)、
      [src/ssh.ts:442](src/ssh.ts)、[src/tunnels.ts:263](src/tunnels.ts)。
...
- [x] **D18 关活动标签可能选中嵌入式会话 → 面板空白** —— `[...tabs.keys()].pop()` 取最后插入的 key，
      ...
      会话。证据：[client-src/index.js:1250-1254](client-src/index.js)、
      [client-src/index.js:1311-1317](client-src/index.js)、
      [client-src/index.js:1656-1660](client-src/index.js)。✅ 复读
...
- [x] **D19 `afterSocketOpen` 无并发 / 代际守卫 + `restoreTab` 不去重** —— 两个触发点
      ...
      证据：[client-src/index.js:4754](client-src/index.js)、[client-src/index.js:4892](client-src/index.js)、
      [client-src/index.js:5140](client-src/index.js)、[client-src/index.js:1135-1154](client-src/index.js)。
...
- [x] **D20 `waitFrame` 把监听挂在全局 socket 上** —— `onMsg` 挂到当次 socket，但清理分支
      ...
      证据：[client-src/index.js:884-905](client-src/index.js)、[client-src/index.js:4859-4865](client-src/index.js)。
...
- [x] **D21 最小化终端面板会取消在途 SFTP 传输** —— `minimizeModal()` 调 `closeSftpDialog()` →
      ...
      取消处理）。证据：[client-src/index.js:5175-5180](client-src/index.js)、
      [client-src/index.js:4662-4674](client-src/index.js)、
      [client-src/index.js:1413-1415](client-src/index.js)。
...
- [x] **D23 非 secure context 下剪贴板未判空 → 局域网访问点「粘贴」直接抛错** ——
      ...
      而 localhost 自测发现不了。证据：[client-src/index.js:5107](client-src/index.js)、
      [client-src/index.js:5112](client-src/index.js)。
...
- [x] **D24 键盘可达性缺口** —— 侧栏入口设了 `role="button"`/`aria-label` 却无 `tabindex`、无
      ...
      证据：[client-src/index.js:5387-5393](client-src/index.js)、
      [client-src/index.js:1722-1725](client-src/index.js)、[client-src/index.js:1020-1025](client-src/index.js)。
...
- [x] **D25 `sftp_remove` 对根目录 / `..` 无任何护栏**（0.19.0 落地：护栏在
      ...
      无二次确认、无干跑。证据：[src/sftp.ts:690-715](src/sftp.ts)、
      [src/index.ts:3428-3430](src/index.ts)。
...
- [x] **D26 下载不存在的路径 / 把目录当文件下载 → 200 + 断流** —— stat 失败被吞（注释「不阻塞下载」），
      ...
      证据：[src/sftp.ts:328-337](src/sftp.ts)、[src/index.ts:2644-2658](src/index.ts)。
...
- [x] **D27 双栏直传在目标栏路径未解析时会写到宿主 cwd** —— pane 初始 `path: ''`，两栏并发
      ...
      证据：[client-src/index.js:3192](client-src/index.js)、[client-src/index.js:3294-3295](client-src/index.js)、
      [client-src/index.js:3619-3620](client-src/index.js)、[client-src/index.js:3372](client-src/index.js)、
      [src/sftp.ts:422-427](src/sftp.ts)。
...
- [x] **D28 Windows 本机栏「..（上级目录）」失效并跳到盘根** —— 只挡住裸盘根，而
      ...
      证据：[client-src/index.js:3347](client-src/index.js)、[client-src/index.js:3745-3750](client-src/index.js)。
...
- [x] **D29 大目录不虚拟滚动，`sftp_list` 无条目上限** —— `renderRows` 对每条目 `appendChild`
      ...
      证据：[client-src/index.js:3345-3366](client-src/index.js)、
      [client-src/index.js:4349-4407](client-src/index.js)、[src/index.ts:3240-3245](src/index.ts)。
...
- [x] **D30 `sftp_read` 的边界问题** —— 二进制嗅探只看**已读前缀**（>256KB 的二进制前段无 NUL 就
      ...
      证据：[src/index.ts:3275](src/index.ts)、[src/index.ts:3290-3293](src/index.ts)。
...
- [x] **D31 `sftp_list` 出参丢 `isSymlink`/`isFile`** —— `list()` 已算出这两个字段，输出 schema 只留
      ...
      证据：[src/index.ts:3216-3230](src/index.ts)、[src/sftp.ts:184-190](src/sftp.ts)。
      修法：至少补 `isSymlink`。
- [x] **D32 本机栏列表完全不排序**（远端是「目录优先 + localeCompare」）→ 同一面板左右两栏规则不一致，
      定位文件靠肉眼扫。证据：[src/index.ts:2034-2055](src/index.ts)、[src/sftp.ts:191-195](src/sftp.ts)。
      修法：复用同一套排序。
- [x] **D33 状态条在窄窗口静默裁掉右侧条目** —— `display:flex` + `overflow:hidden` +
      `white-space:nowrap`、条目 `flex:0 0 auto` → 溢出直接裁掉，10 个条目都没有 `title`，全文件只有一处
      `prefers-reduced-motion` 媒体查询。证据：[client-src/tty.css:1007-1035](client-src/tty.css)、
      [client-src/index.js:636-651](client-src/index.js)。
...
- [x] **D34 标签持久化载荷无版本字段 + `ready` 帧打断行内重命名** —— `loadPersistedTabs()` 只校验
      ...
      证据：[client-src/index.js:868-878](client-src/index.js)、[client-src/index.js:818-832](client-src/index.js)、
      [client-src/index.js:4915-4918](client-src/index.js)、[client-src/index.js:1940-1972](client-src/index.js)。
...
- [x] **D35 三处弹窗用局部 `const setStatus` 遮蔽模块级同名函数** —— `openSshDialog` /
      ...
      证据：[client-src/index.js:2582](client-src/index.js)、[client-src/index.js:3149](client-src/index.js)、
      [client-src/index.js:4244](client-src/index.js)。修法：重命名 `setDialogStatus` / `setPaneStatus`。
- [x] **D36 `tunnel_list` 出参 schema 与实现不符** —— schema 声明 `additionalProperties:false` 且只列
      ...
      证据：[src/index.ts:3156-3191](src/index.ts)、[src/tunnels.ts:144-157](src/tunnels.ts)。
...
- [x] **D37 零碎但确凿的四条** —— ①zsh 桩目录缺 `.zlogout`（ZDOTDIR 指过去后用户的退出钩子被静默吞掉）；
      ...
      证据：[src/shell-integration.ts:177-183](src/shell-integration.ts)、
      [src/shell-integration.ts:322-326](src/shell-integration.ts)、[src/index.ts:1732](src/index.ts)、
      [src/known-hosts.ts:113](src/known-hosts.ts)。
...
- [x] **D39 CI 不跑旗舰脚本，也没有「产物与源码一致」闸门** —— `integration.mjs`、
      ...
      只覆盖 4 个聚合文件）。
      证据：[.github/workflows/ci.yml:56](../../.github/workflows/ci.yml)、[scripts/client-lint.mjs:45](../../scripts/client-lint.mjs)。
...
- [x] **D40 `integration.mjs` 的失败信息掩盖真因** —— 受限环境里 PTY spawn 被拒时（`posix_openpt
      ...
      证据：[scripts/integration.mjs:104](scripts/integration.mjs)。
...
- [x] **D43 `preview.mjs` 默认重建 `client.js`（隐式写入库产物）** —— 默认走 build（`--no-build` 才跳过），
      ...
      改掉必须入库的文件。证据：[scripts/preview.mjs:209](scripts/preview.mjs)、
      [scripts/preview.mjs:211-212](scripts/preview.mjs)、[scripts/preview.mjs:446](scripts/preview.mjs)。
