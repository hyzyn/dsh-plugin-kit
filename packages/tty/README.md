...
- **固定槽位 + 增量刷新**：每个值的字符槽位固定（右对齐，最宽的一档照样只占
  自己那一格），条目只建一次、之后只写真的变了的文本。所以 `CPU 5% → 12%`、
...
——标题写着 `SFTP · lab-b`、底下活动标签却是 `192.0.2.10`，面板里躺着**另一台
