...
- **名字**：由「连接簿名称」派生，**就显示在字段里**，想换成好记的直接改。纯 ASCII 名字得到
  `DSH_TTY_<名字>_PASSWORD`；**名字含中文等非 ASCII 字符时会额外拼一段由完整名字派生的短哈希**
  （如 `DSH_TTY_HK_B075E02B_PASSWORD`）——引用文法只认 ASCII 标识符，不做这一步的话中文名会被
  清空、**所有中文名的连接都塌到同一个引用上互相覆盖**（实测发现的坑）。名字为空则拒绝存入。
  改连接名不会回头改已存的引用（会留下孤儿，用「清除已存凭据」按字段里的引用清掉）。
