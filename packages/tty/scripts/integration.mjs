...
async function run() {
  ...
  console.log('\n[23] 会话持久化（tmux）')
  {
    ...
    if (!hasTmux) {
      ...
      // sessions 帧带 tmux 会话名清单（新窗口恢复持久标签的存活确认数据源）
      const wq = openSession(port)
      await wq.open()
      wq.client.send(JSON.stringify({ t: 'sessions' }))
      await wq.waitFor(() => wq.state.frames.some((f) => f.t === 'sessions'), 5000, 'sessions 帧')
      const sframe = wq.state.frames.find((f) => f.t === 'sessions')
      if (sframe !== undefined && Array.isArray(sframe.tmux) && sframe.tmux.includes('dsh-b26resume')) pass('B26a2 sessions 帧带 tmux 会话名清单')
      else fail('B26a2 sessions 帧带 tmux 会话名清单', JSON.stringify(sframe?.tmux))
      wq.client.close()
