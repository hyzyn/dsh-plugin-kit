 ...
 */
export function buildCommandSpawn(shell, term, colorTerm, command, platform = process.platform) {
    if (platform === 'win32')
        return buildWindowsCommandSpawn(shell, command);
 ...
 */
export function defaultShellPath(platform = process.platform, env = process.env) {
    if (platform === 'win32')
 ...
 */
function buildWindowsShellSpawn(shell) {
...
function buildWindowsCommandSpawn(shell, command) {
    return isPowerShellShell(shell)
        ? { argv: [shell, '-NoLogo', '-Command', command], env: {} }
        : { argv: [shell, '/c', command], env: {} };
}
export function buildShellSpawn(shell, term, colorTerm, integration, platform = process.platform) {
    if (platform === 'win32')
        return buildWindowsShellSpawn(shell);
    const pre = `export TERM='${term}'; export COLORTERM='${colorTerm}';`;
