...
class TtyServer {
    ...
    setStatsSub(session, clientSid, on) {
        if (on) {
    ...
    pruneStatsSubs(session) {
        for (const clientSid of [...session.statsSubs]) {
            if (!session.clients.has(clientSid))
                this.setStatsSub(session, clientSid, false);
        }
    }
    /**
    ...
    rebindClient(session, sid, ws, local) {
