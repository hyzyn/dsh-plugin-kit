...
function matchHashedToken(token, candidates) {
...
export function parseKnownHostsDetailed(text, candidates = []) {
    ...
    outer: for (const rawLine of text.split(/\r?\n/)) {
        ...
        if (tokens[0].startsWith('@'))
            continue; // @cert-authority / @revoked 不入库
