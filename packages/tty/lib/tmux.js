...
export async function probeTmux() {
    if (probeCache !== null && Date.now() - probeCache.at < PROBE_TTL_MS)
...
export function sanitizePersistName(raw, sid) {
...
function buildTmuxConf(passthrough, runtimeDir) {
    const lines = [
    ...
    if (passthrough)
        lines.push('set -g allow-passthrough on');
    return lines.join('\n') + '\n';
}
/**
 ...
 */
export function ensureTmuxAssets(options) {
 ...
 */
export function buildTmuxSpawnPlan(options) {
    const runtimeDir = pluginRuntimeDir();
...
export function killTmuxSession(tmuxName) {
...
export function refreshTmuxClient(tmuxName) {
    return new Promise((resolve) => {
        execFile('tmux', ['-L', TMUX_SOCKET, 'list-clients', '-t', tmuxName, '-F', '#{client_name}'], { timeout: 4000 }, (error, stdout) => {
            ...
            void Promise.all(clients.map((client) => new Promise((done) => {
                execFile('tmux', ['-L', TMUX_SOCKET, 'refresh-client', '-t', client], { timeout: 4000 }, () => done());
            }))).then(() => resolve());
        });
    });
}
