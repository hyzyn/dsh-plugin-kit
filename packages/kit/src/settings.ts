...
/** schemastery volatile 字段的解析结果：冻结引用，只能经 `get()` 取值。 */
interface VolatileRef {
  get(): unknown
}

/** DSH ≥0.1.7 settings 服务（SettingsForms）的最小类型面。 */
 ...
 */
export function settingsEntryId(ctx: Context, fallbackNs: string): string {
...
function unwrap(value: unknown, seen: WeakMap<object, unknown>): unknown {
  ...
  if (Array.isArray(value)) {
    const out: unknown[] = []
    seen.set(value, out)
    for (const item of value) out.push(unwrap(item, seen))
    return out
  }
  ...
  seen.set(value, out)
  for (const [key, item] of Object.entries(value)) out[key] = unwrap(item, seen)
  return out
}
...
function describeEntry(settings: SettingsFormsLike, ns: string): Record<string, unknown> | undefined {
  try {
 ...
 */
export function settingsEntryScope(ctx: Context, fallbackNs: string): SettingsEntryScope | undefined {
 ...
 */
export function readSettingsEntry(ctx: Context, ns: string): Record<string, unknown> | undefined {
  const settings = settingsService(ctx)
 ...
 */
export function suppressAutoSettingsPage(settingsCtx: Context, owner: Context): () => void {
  const settings = settingsService(settingsCtx)
  if (settings?.configure === undefined) return () => {}
  const fiber = (owner as unknown as { fiber?: unknown }).fiber
