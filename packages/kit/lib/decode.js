 ...
 */
export function consoleEncoding() {
    if (cachedConsoleEncoding === undefined)
        cachedConsoleEncoding = detectConsoleEncoding();
...
/** 供测试重置进程内缓存（真机上不必调用）。 */
export function resetConsoleEncodingCache() {
 ...
 */
export function createOutputDecoder(options = {}) {
    ...
    return {
        decode(chunk) {
            ...
            if (buf.length === 0)
                return '';
            if (mode === 'fallback')
                return fallbackDecoder().decode(buf, { stream: true });
            keepReplay(buf);
            try {
 ...
 */
export function decodeOutput(buf, options = {}) {
