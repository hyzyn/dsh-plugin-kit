...
import type { ExecLogger, HostKeyRecord, HostKeyStore, SshSpec } from './ssh-exec.js'
...
const plugin = definePlugin<Config>({
  ...
  apply(ctx: Context, config?: Config) {
    ...
    const ttySeed = (): Map<string, string> => {
      ...
      for (const record of readTtyHostKeys(settingsApi)) map.set(`${record.host}:${String(record.port)}`, record.fingerprint)
      return map
    }
    ...
    const hostKeyStore: HostKeyStore = {
      get(host, port) {
        ...
        if (own !== undefined) return own.fingerprint
    ...
    const pickTarget = (input: unknown): { name?: string; error?: string } => {
      ...
      const list = targetsNow()
      if (list.length === 0) return { error: '尚未配置任何 Docker 目标（设置 → 插件 → Docker 容器面板）' }
    ...
    const renderTargets = (rows: Array<{ name: string; kind: string; label: string; ok?: boolean; error?: string }>): string => {
      if (rows.length === 0) return '尚未配置任何 Docker 目标（设置 → 插件 → Docker 容器面板 → 目标）。'
    ...
    const refreshTools = (): void => {
      ...
      if (!live.enabled) return
      const targetParam = { type: 'string', description: '目标名（docker_targets 列出；只有一个目标时可省略）' } as const
      ...
      add('docker_targets', defineTool({
        ...
        output: {
          ...
          render: (_args: unknown, value: unknown) => {
            const rows = (value as { targets?: Array<{ name: string; kind: string; label: string; ok?: boolean; error?: string }> }).targets ?? []
            return [{ type: 'text', text: renderTargets(rows) }]
          },
        },
        async execute(args: unknown): Promise<{ targets: Array<{ name: string; kind: string; label: string; ok?: boolean; error?: string }> }> {
          const input = (args ?? {}) as { probe?: unknown }
          const books = readTtyBooks(settingsApi)
          const rows: Array<{ name: string; kind: string; label: string; ok?: boolean; error?: string }> = []
      ...
      add('docker_ps', defineTool({
        ...
        output: {
          ...
          render: (_args: unknown, value: unknown) => {
            const v = value as { target?: string; containers?: ContainerSummary[] }
            return [{ type: 'text', text: renderContainers(v.target ?? '?', v.containers ?? []) }]
          },
        },
        async execute(args: unknown): Promise<{ target: string; containers: ToolPsRow[] }> {
      ...
      if (live.allowMutations) {
        add('docker_action', defineTool({
          ...
          async execute(args: unknown): Promise<{ target: string; id: string; action: string; message: string }> {
            if (!live.allowMutations) throw new Error('变更操作未启用（设置 → 插件 → Docker 容器面板 → 允许变更操作）')
      ...
      if (live.allowExec) {
        add('docker_exec', defineTool({
          ...
          async execute(args: unknown): Promise<ToolExecRow> {
            if (!live.allowExec) throw new Error('exec 未启用（设置 → 插件 → Docker 容器面板 → 允许 exec）')
          ...
          const handlers: StreamHandlers = {
            onStdout: (chunk) => {
              ...
              for (const raw of found) {
                ...
                lastRaw = raw
                for (const row of parseStatsJson(raw)) sendEvent('stats', row)
              }
            },
            onStderr: (chunk) => sendEvent('line', { e: chunk }),
          }
        ...
        const dispose = webServer.register({
          ...
          handler: async (req: ReqLike & AsyncIterable<Uint8Array>, res: ResLike) => {
            ...
            if (sub === '/config') {
              ...
              if (patch.targets !== undefined) patch.targets = mergeTargetSecrets(targetsNow(), patch.targets)
              const scope = settingsScope
              if (scope !== undefined) {
                try {
