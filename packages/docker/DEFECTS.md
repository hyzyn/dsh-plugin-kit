...
> **这是一份时点记录。** 2026-09-19 对 v0.6.4（HEAD `44636e61`）做了两轮**只读**审计：第一轮 8 路并行
...
> 证据、触发场景与修法（约 1200 行）冻结在 git 历史里：`git show 5254d93d:packages/docker/DEFECTS.md`（1201 行，含审计原文与两轮修复记录）。
...
**已修 126 / 待修 0**（P1×6、P2×37、P3×83；第一轮 79 + 第二轮 46 + 线上实测 1）。
...
> 逐条证据与修法见 `git show 5254d93d:packages/docker/DEFECTS.md`；这里只留「改成了什么」，
