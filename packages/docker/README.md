...
- **日志视图（紧凑两行）**：第一行 = 返回 + 容器名 + 状态 + 目标主机 +
  ...
  都能识别），时间戳压暗，过滤命中高亮；超大日志只渲染最近 400 行（DOM 渲染窗口）并提示，导出最多 2000 行。
...
- **FOLLOW 实时日志流**：日志页 `FOLLOW` 开关打开后，界面从「定时拉快照」切换
  ...
  渲染（话痨容器也不再逐条重绘），同一时刻只把**最近 400 行**挂进 DOM（渲染窗口，
  过滤 / 级别着色与快照共用一套渲染；导出仍最多 2000 行）。自动滚动到底部，用户向上滚动时暂停并浮出
