...
| 面板承载 | **默认走会话右侧栏标签**（`sidebar.right.pane.tab`）：面板与对话同屏，折叠即退出视野、不占屏；入口（侧边栏「容器」/ tty 连接栏）只负责打开或聚焦它，**页类型在同一栏内去重**，反复点不会开出第二个。拿不到右侧栏服务（老版本 DSH）、或把 `localStorage['dsh-docker:carrier']` 置成 `modal` 时回退到原路径：tty ≥ 0.16 且终端面板开着时经 `ttyPanel.mountPane` 挂在终端右侧的 dock（拖宽 / 折叠 / ✕ 由 tty 提供），否则自带 backdrop 的全屏模态。三种承载是**同一个组件**，只是外壳与几何不同 |
...
- **SSH 连接栏「容器」按钮**（上下文快捷方式，tty ≥ 0.13.0）：在 tty 终端面板的
  ...
  **当前会话那台主机**打开面板，不用再选目标。目标解析发生在点击时：会话来自
  ...
  这条路径带目标进标签，**换目标即重挂面板**（状态归零，正是「我要看这台主机」的
  预期）；不带目标的总入口则沿用面板上次的状态。
...
| **终端右侧 dock** | 拿不到右侧栏服务，或 `localStorage['dsh-docker:carrier'] = 'modal'`，且 tty ≥ 0.16 面板开着 | 挂在终端面板右侧（拖宽 / 折叠 / ✕ 由 tty 提供，终端继续可用）；同一时刻只挂一个，挂上去会收掉前一个（例如 tty 自己的 SFTP） |
