...
- **日志右键交给 Agent + 面板内 Agent 抽屉**：日志页拖选报错行 → 右键「预览后发送 / 直接发送到当前会话 / 只填入输入框」，把「选中的行 + 目标 / 容器 / 时间窗 + 前后各 20 行上下文」交给会话（内容会进模型上下文，菜单里明写了留意凭证）。面板底部是一块 **Agent 抽屉**：默认收起、发送后自动展开，**实时跟随回复**——推理 / 正文 / 工具调用按到达顺序落进来，状态行给「正在分析 / 已完成 / 已中断」。数据取自会话事件窗口（`sessions.binding(id).eventSource`，含 token 级增量），所以**三种承载下都成立**：发完日志不必离开面板就能看它干活。
...
| 面板承载 | **默认走会话右侧栏标签**（`sidebar.right.pane.tab`）：面板与对话同屏，折叠即退出视野、不占屏；**框架侧边栏**的入口只负责打开或聚焦它，**页类型在同一栏内去重**，反复点不会开出第二个。**终端连接栏**的入口相反——它长在盖满视口的 tty 弹窗上，开标签会被挡住，所以那条走 `ttyPanel.mountPane` 停靠到终端右侧（**入口决定承载**）。拿不到右侧栏服务（老版本 DSH）、或把 `localStorage['dsh-docker:carrier']` 置成 `modal` 时，侧边栏入口也回退到 dock（tty 开着时）或自带 backdrop 的全屏模态。三种承载是**同一个组件**，只是外壳与几何不同 |
...
- **Agent 抽屉**：面板底部那条 `Agent` 标题栏。默认收起；用日志右键投递后会**自动展开**
  并跟着回复滚。状态点的颜色对应「正在分析（蓝）/ 已完成（绿）/ 已中断（红）/ 拿不到事件源（黄）」。
  docked / 模态承载下它还会提示「会话在面板后面」——tty 面板没有关闭或最小化的接口，
  我们关不掉它，只能指个方向。
