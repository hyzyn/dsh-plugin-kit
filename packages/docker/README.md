...
- **日志视图（紧凑两行）**：第一行 = 返回 + 容器名 + 状态 + 目标主机 +
  ...
  （`全部级别 / WARN+ / ERROR+`）+ **导出**（`⬇ .log` / `⬇ .md`，导出的是**当前显示内容**）
